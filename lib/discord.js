...
const { Discord, bot } = require('./discord_client.js');
...
class DiscordBot extends ChatBot {
	...
	instantiateChatHooks() {
		bot.on('message', (msg) => {
			...
			this.onCommand(msg, 'verify', (code) => {
				this.logger.log('verify called with code:', code);
				this.logger.log(msg.id);
		...
		bot.on('guildMemberAdd', async (member) => {
			...
			let welcome = new Discord.RichEmbed()
					...
					.addField('You can join the community here', dependency.invite_link)
	...
	async refreshInformation() {
		...
		if (invites.size) {
			information.invite_link = `https://discord.gg/${invites.values().next().value.code}`
		}
